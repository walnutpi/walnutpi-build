...
generate_kernel_headers() {
    ...
    (
        ...
        find arch/$CHIP_ARCH -name module.lds -o -name Kbuild.platforms -o -name Platform
        find $(find arch/$CHIP_ARCH -name include -o -name scripts -type d) -type f
    ) > debian/hdrsrcfiles
    
    {
        ...
        find arch/$CHIP_ARCH/include Module.symvers include scripts -type f
...
compile_kernel() {
    ...
    # make  CROSS_COMPILE=$FILE_CROSS_COMPILE ARCH=${CHIP_ARCH} $LINUX_CONFIG
    # make  CROSS_COMPILE=$FILE_CROSS_COMPILE ARCH=${CHIP_ARCH} -j$thread_count
...
#     TMP_HEADER_DEB=$PATH_TMP/linux_headers_${LINUX_BRANCH}
#     cd $PATH_KERNEL
#     # make clean
#     # make clean CROSS_COMPILE=$FILE_CROSS_COMPILE ARCH=${CHIP_ARCH}
#     generate_kernel_headers ${TMP_HEADER_DEB}
    
#     DEB_HEADERS_NAME=${PACKAGE_HEADERS_NAME}_${deb_version}_all.deb
    
#     mkdir $TMP_HEADER_DEB/DEBIAN
#     cat << EOF > $TMP_HEADER_DEB/DEBIAN/control
# Package: ${PACKAGE_HEADERS_NAME}
# Description: linux kernel file
# Maintainer: ${git_email}
# Version: ${deb_version}
# Section: free
...
# Architecture: all
# EOF
#     run_status "创建deb包 ${DEB_HEADERS_NAME} " dpkg -b "$TMP_HEADER_DEB" "${PATH_KERNEL_PACKAGE}/${DEB_HEADERS_NAME}"
    
#     # run_status "导出linux相关包"  make  CROSS_COMPILE=$FILE_CROSS_COMPILE ARCH=${CHIP_ARCH} -j$thread_count $LINUX_CONFIG bindeb-pkg BRANCH=current  KDEB_PKGVERSION=$(cat ${PATH_PWD}/VERSION)
#     # run_status "导出linux相关包"  make  CROSS_COMPILE=$FILE_CROSS_COMPILE ARCH=${CHIP_ARCH} -j$thread_count $LINUX_CONFIG bindeb-pkg BRANCH=current  KDEB_PKGVERSION=$(cat ${PATH_PWD}/VERSION) LOCALVERSION=-${BOARD_NAME_SMALL}
#     # 只想要herders这个包,其他的暂时不用
#     mv ../*headers*.deb ${PATH_KERNEL_PACKAGE}/
#     rm -r ../*.deb
