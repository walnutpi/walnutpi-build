...
pack_all_img() {
    ...
    local TMP_IMG_DISK1="${PATH_TMP}/PART1-${BOARD_NAME}-${ENTER_os_ver}_${ENTER_rootfs_type}"
    local TMP_IMG_DISK2="${PATH_TMP}/PART2-${BOARD_NAME}-${ENTER_os_ver}_${ENTER_rootfs_type}"
    ...
    local MAPPER_DEVICE1="/dev/mapper/${MAPPER_DEVICE}p1"
    local MAPPER_DEVICE2="/dev/mapper/${MAPPER_DEVICE}p2"
    ...
    local BOOT_UUID=$(blkid -s UUID -o value $MAPPER_DEVICE1)
    local BOOT_PARTUUID=$(blkid -s PARTUUID -o value $MAPPER_DEVICE1)
    local ROOTFS_UUID=$(blkid -s UUID -o value $MAPPER_DEVICE2)
    ...
    local NEW_IMG_FILE_NAME=$NEW_IMG_FILE_NAME_PARAM
